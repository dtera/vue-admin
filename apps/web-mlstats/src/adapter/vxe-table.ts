  ...
  configVxeTable: (vxeUI) => {
    ...
    vxeUI.renderer.add('CellImage', {
      renderTableDefault({ props }, { column, row }) {
        return h(Image, { src: row[column.field].split(',')[0], ...props });
      },
    });

    // 表格配置项可以用 cellRender: { name: 'CellJsonPretty' },
    vxeUI.renderer.add('CellJsonPretty', {
      renderTableDefault({ props }, { column, row }) {
        return h(VueJsonPretty, {
          data: JSON.parse(row[column.field]),
          ...props,
        });
      },
    });
